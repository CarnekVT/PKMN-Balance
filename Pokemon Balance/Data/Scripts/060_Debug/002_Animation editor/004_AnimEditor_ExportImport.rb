...
module BattleAnimationEditor
  ...
  def pbExportAnim(animations)
    filename = pbMessageFreeText(_INTL("Ingrese un nombre de archivo."), "", false, 32)
    if filename != ""
      begin
        filename += ".anm"
        ...
        pbMessage(_INTL("No se pudo guardar la animación en {1}.", filename))
        failed = true
      end
      if !failed
        pbMessage(_INTL("La animación se guardó en {1} en la carpeta del juego.", filename))
        pbMessage(_INTL("Es un archivo de texto, por lo que se puede transferir fácilmente a otros."))
      end
    end
  end
    ...
    loop do
      ...
      if Input.trigger?(Input::USE) && animfiles.length > 0
        begin
          ...
          pbMessage(_INTL("La animación es inválida o no se pudo cargar."))
          next
        end
        ...
        if graphic && graphic != "" && !FileTest.image_exist?(graphic)
          pbMessage(_INTL("El archivo de animación {1} no fue encontrado. La animación se cargará de todos modos.", graphic))
        end
  ...
  def pbConvertAnimsToNewFormat
    pbMessage(_INTL("Se convertirán las animaciones ahora."))
    count = 0
    animations = pbLoadBattleAnimations
    if !animations || !animations[0]
      pbMessage(_INTL("No hay animaciones."))
      return
    end
    ...
    pbMessage(_INTL("{1} animaciones convertidas al nuevo formato.", count))
  end
end