...
module AnimationPlayer::Helper
  ...
  def set_bitmap_and_origin(particle, sprite, user_index, target_index, user_sprites, target_sprites)
    ...
    when "USER", "USER_OPP", "USER_FRONT", "USER_BACK",
      ...
      if filename.is_a?(DeluxeBitmapWrapper)
        sprite.bitmap = filename.bitmap
        ...
        frame_width = sprite.bitmap.width / frames
        frame_height = sprite.bitmap.height
        sprite.src_rect.set(0, 0, frame_width, frame_height)
        sprite.ox = frame_width / 2
        sprite.oy = frame_height
      end
    else
      sprite.bitmap = RPG::Cache.load_bitmap("Graphics/Animations/", particle[:graphic])
      sprite.src_rect.set(0, 0, sprite.bitmap.width, sprite.bitmap.height)
