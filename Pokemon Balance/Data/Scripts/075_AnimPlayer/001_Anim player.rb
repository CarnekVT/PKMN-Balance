...
class AnimationPlayer
  ...
  def set_up_particle(particle, target_idx = -1, instance = 0)
    ...
    if sprite
      ...
      # Stop battler sprite animation
      sprite.animating = false if particle_sprite.battler_sprite? && sprite.respond_to?(:animating=)
    end
    ...
    # Set editor zoom for USER/TARGET
    if sprite
    ...
    # For USER/TARGET, remove zoom properties to maintain editor zoom
    if ["USER", "USER_OPP", "USER_FRONT", "USER_BACK", "TARGET", "TARGET_OPP", "TARGET_FRONT", "TARGET_BACK"].include?(particle[:graphic])
      particle.delete(:zoom_x)
      particle.delete(:zoom_y)
    end
    # Add all commands
  ...
  def set_up
    ...
    reset_anim_sprites
    # Set editor zoom for USER/TARGET battlers
    @anim_sprites.each do |particle|
      next if !particle.sprite || !particle.battler_sprite?
      if particle.sprite == @sprites["pokemon_#{@user&.index}"]
        particle.sprite.zoom_x = 3.0
        particle.sprite.zoom_y = 3.0
      elsif @targets && @targets.any? { |t| particle.sprite == @sprites["pokemon_#{t.index}"] }
        particle.sprite.zoom_x = 2.0
        particle.sprite.zoom_y = 2.0
      end
    end
  end
