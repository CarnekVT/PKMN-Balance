...
module Compiler
  @@categories[:animations] = {
    ...
    :header_text    => proc { next _INTL("Compilando animaciones") },
    :skipped_text   => proc { next _INTL("No compilado") },
    :compile        => proc {
      # Delete old data files in preparation for recompiling
      begin
        File.delete(File.join("Data", "animations.dat")) if FileTest.exist?(File.join("Data", "animations.dat"))
  ...
  def get_animation_pbs_files_to_compile
    ret = []
    if FileTest.directory?(File.join("PBS", "Animations"))
      Dir.all(File.join("PBS", "Animations"), "**/**.txt").each { |file| ret.push(file) }
    end
    return ret
  end
  ...
  def should_compile_animations?
    ...
    if FileTest.exist?(File.join("Data", data_file))
      begin
        File.open(File.join("Data", data_file)) do |file|
  ...
  def compile_battle_animations(*paths)
    ...
    Console.echo_li(_INTL("Compilando archivos PBS de animaciones..."))
    paths.each do |path|
      ...
      pbCompilerEachPreppedLine(path) do |line, line_no|
        ...
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]
          ...
          if schema[key]   # Property of the animation
            ...
            if !current_particle
              raise _INTL("La partícula no ha sido definida aún.") + "\n" + FileLineData.linereport
            end
  ...
  def validate_compiled_animation(hash)
    ...
    ["User", "Target", "SE"].each do |type|
      ...
      raise _INTL("La animación tiene más de 1 partícula \"{1}\", lo cual no está permitido.", type) + "\n" + FileLineData.linereport
    end
    # Ensure there is no "User" particle if "NoUser" is set
    if hash[:particles].any? { |particle| particle[:name] == "User" } && hash[:no_user]
      raise _INTL("No se puede definir una partícula \"User\" y también establecer la propiedad \"NoUser\" como verdadera.") + "\n" + FileLineData.linereport
    end
    # Ensure there is no "Target" particle if "NoTarget" is set
    if hash[:particles].any? { |particle| particle[:name] == "Target" } && hash[:no_target]
      raise _INTL("No se puede definir una partícula \"Target\" y también establecer la propiedad \"NoTarget\" como verdadera.") + "\n" + FileLineData.linereport
    end
    ...
    hash[:particles].each do |particle|
      ...
      if particle[:name] == "SE"
        particle.keys.each do |property|
          ...
          raise _INTL("La partícula \"{1}\" tiene un comando que no es un comando de tipo \"Play\".",
            particle[:name]) + "\n" + FileLineData.linereport
        end
      else
        if particle[:se]
          raise _INTL("La partícula \"{1}\" tiene un comando \"Play\" pero no debería.",
                      particle[:name]) + "\n" + FileLineData.linereport
        elsif particle[:user_cry]
          raise _INTL("La partícula \"{1}\" tiene un comando \"PlayUserCry\" pero no debería.",
                      particle[:name]) + "\n" + FileLineData.linereport
        elsif particle[:target_cry]
          raise _INTL("La partícula \"{1}\" tiene un comando \"PlayTargetCry\" pero no debería.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
      end
      ...
      if hash[:no_user]
        if GameData::Animation::FOCUS_TYPES_WITH_USER.include?(particle[:focus])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Focus\" que involucre a un usuario si la propiedad \"NoUser\" está establecida como verdadera.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if ["USER", "USER_OPP", "USER_FRONT", "USER_BACK"].include?(particle[:graphic])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Graphic\" que involucre a un usuario si la propiedad \"NoUser\" está establecida como verdadera.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:name] == "SE" && particle[:user_cry] && !particle[:user_cry].empty?
          raise _INTL("La animación no puede reproducir el grito del usuario si la propiedad \"NoUser\" está establecida como verdadera.") + "\n" + FileLineData.linereport
        end
      end
      ...
      if hash[:no_target]
        if GameData::Animation::FOCUS_TYPES_WITH_TARGET.include?(particle[:focus])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Focus\" que involucre a un objetivo si la propiedad \"NoTarget\" está establecida como verdadera.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if ["TARGET", "TARGET_OPP", "TARGET_FRONT", "TARGET_BACK"].include?(particle[:graphic])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Graphic\" que involucre a un objetivo si la propiedad \"NoTarget\" está establecida como verdadera.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:name] == "SE" && particle[:target_cry] && !particle[:target_cry].empty?
          raise _INTL("La animación no puede reproducir el grito del objetivo si la propiedad \"NoTarget\" está establecida como verdadera.") + "\n" + FileLineData.linereport
        end
      end
      ...
      if GameData::Animation::FOCUS_TYPES_WITH_USER.include?(particle[:focus]) == GameData::Animation::FOCUS_TYPES_WITH_TARGET.include?(particle[:focus])
        if particle[:foe_invert_x]
          raise _INTL("La partícula \"{1}\" no puede establecer \"FoeInvertX\" si su enfoque no es exactamente 1 cosa.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:foe_invert_y]
          raise _INTL("La partícula \"{1}\" no puede establecer \"FoeInvertY\" si su enfoque no es exactamente 1 cosa.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:foe_flip]
          raise _INTL("La partícula \"{1}\" no puede establecer \"FoeFlip\" si su enfoque no es exactamente 1 cosa.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
      end
      ...
      if (particle[:angle_override] || :none) != :none &&
        ...
        raise _INTL("La partícula \"{1}\" no puede establecer \"AngleOverride\" si su enfoque no es una cosa específica.",
                    particle[:name]) + "\n" + FileLineData.linereport
      end
      ...
      if !["User", "Target", "SE"].include?(particle[:name]) &&
        ...
        raise _INTL("La partícula \"{1}\" no puede tener ningún comando \"Frame\" si su gráfico es el sprite de un Pokémon.",
                    particle[:name]) + "\n" + FileLineData.linereport
      end
        ...
        [:se, :user_cry, :target_cry].each do |property|
          ...
          particle[property].each do |play|
            ...
            if files_played[play[0]].include?(play[1])
              case property
            #   when :se
            #     raise _INTL("SE \"{1}\" no debería reproducirse dos veces en el mismo fotograma ({2}).", play[1], play[0]) + "\n" + FileLineData.linereport
              when :user_cry
                raise _INTL("El grito del usuario no debería reproducirse dos veces en el mismo frame ({1}).", play[0]) + "\n" + FileLineData.linereport
              when :target_cry
                raise _INTL("El grito del objetivo no debería reproducirse dos veces en el mismo frame ({1}).", play[0]) + "\n" + FileLineData.linereport
              end
            end
      ...
      particle.keys.each do |key|
        ...
        particle[key].each do |cmd|
          # if last_frame > cmd[0]
          #   raise _INTL("La animación tiene comandos superpuestos para la propiedad {1}.",
          #               key.to_s.capitalize) + "\n" + FileLineData.linereport
          # end
        #   if cmd[1] == 0 && last_set_frame >= cmd[0]
        #     raise _INTL("La animación tiene múltiples comandos \"Set\" en el mismo fotograma clave para la propiedad {1}.",
        #                 key.to_s.capitalize) + "\n" + FileLineData.linereport
        #   end
      ...
      if particle[:blending]
        particle[:blending].each do |blend|
          next if blend[2] <= 2
          raise _INTL("Valor de mezcla no válido: {1} (debe ser 0, 1 o 2).\n{2}",
