...
class AnimationEditor::Canvas < Sprite
  ...
  def prepare_to_play_animation
    ...
    if particle_idx
      ...
      @battler_sprites[idx].to_frame(0) if @battler_sprites[idx].respond_to?(:to_frame)
      @battler_sprites[idx].zoom_x = 3.0
      @battler_sprites[idx].zoom_y = 3.0
    end
    ...
    if particle_idx
      target_indices.each do |idx|
        ...
        AnimationPlayer::Helper.apply_z_focus_to_sprite(@battler_sprites[idx], 0, focus_z)
        @battler_sprites[idx].to_frame(0) if @battler_sprites[idx].respond_to?(:to_frame)
        @battler_sprites[idx].speed = 0 if @battler_sprites[idx].respond_to?(:speed)
        @battler_sprites[idx].zoom_x = 2.0
        @battler_sprites[idx].zoom_y = 2.0
      end
    end
    hide_all_sprites
  ...
  def end_playing_animation
    ...
    @playing = false
    @keyframe = 0
    # Reset battler sprites to initial state
    if !@anim[:no_user]
      idx = user_index
      spr = @battler_sprites[idx]
      if spr && !spr.disposed?
        spr.x = @user_coords[0]
        spr.y = @user_coords[1]
        spr.zoom_x = 3.0
        ...
        spr.tone.set(0, 0, 0, 0)
        spr.src_rect.x = 0
        spr.speed = 1 if spr.respond_to?(:speed)
        spr.animating = true if spr.respond_to?(:animating=)
      end
    end
    if !@anim[:no_target]
      target_indices.each do |idx|
        spr = @battler_sprites[idx]
        if spr && !spr.disposed?
          spr.x = @target_coords[idx][0]
          spr.y = @target_coords[idx][1]
          spr.zoom_x = 2.0
          spr.zoom_y = 2.0
          spr.angle = 0
          spr.mirror = false
  ...
  def refresh_bg_graphics
    ...
    # Scale the canvas to fit the viewport for better preview
    if self.bitmap
      self.zoom_x = self.viewport.rect.width.to_f / self.bitmap.width
      self.zoom_y = self.viewport.rect.height.to_f / self.bitmap.height
    end
  ...
  def refresh_battler_graphics
    if !@user_sprite_name || !@user_sprite_name || @user_sprite_name != @settings[:user_sprite_name]
      ...
      @user_bitmap_front&.dispose
      @user_bitmap_back&.dispose
      @user_bitmap_front = GameData::Species.front_sprite_bitmap(@user_sprite_name, 0, 0, false, false)
      @user_bitmap_back = GameData::Species.back_sprite_bitmap(@user_sprite_name, 0, 0, false, false)
    end
    if !@target_bitmap_front || !@target_sprite_name || @target_sprite_name != @settings[:target_sprite_name]
      ...
      @target_bitmap_front&.dispose
      @target_bitmap_back&.dispose
      @target_bitmap_front = GameData::Species.front_sprite_bitmap(@target_sprite_name, 0, 0, false, false)
      @target_bitmap_back = GameData::Species.back_sprite_bitmap(@target_sprite_name, 0, 0, false, false)
    end
  end
  ...
  def recalculate_battler_position(index, size, sprite_name, btmp)
    ...
    height = btmp.respond_to?(:frame_height) ? btmp.frame_height : btmp.height
    return [spr.x, spr.y - (height / 2)]
  end
  ...
  def refresh_sprite(index, target_idx = -1)
    ...
    AnimationPlayer::Helper.set_bitmap_and_origin(particle, spr, user_index, target_idx,
                                                   [@user_bitmap_front, @user_bitmap_back],
                                                   [@target_bitmap_front, @target_bitmap_back])
    offset_xy = AnimationPlayer::Helper.get_xy_offset(particle, spr)
    # Set src_rect to first frame and zoom for Pokemon sprites
    if spr.bitmap && ["USER", "USER_OPP", "USER_FRONT", "USER_BACK", "TARGET", "TARGET_OPP", "TARGET_FRONT", "TARGET_BACK"].include?(particle[:graphic])
      frames = spr.bitmap.width / spr.bitmap.height
      spr.src_rect = Rect.new(0, 0, spr.bitmap.width / frames, spr.bitmap.height)
      if particle[:graphic].include?("USER")
        spr.zoom_x = 3.0
        spr.zoom_y = 3.0
      elsif particle[:graphic].include?("TARGET")
        spr.zoom_x = 2.0
        spr.zoom_y = 2.0
      end
    end
    ...
    spr.y += offset_xy[1]
    # Set frame
    if ["USER", "USER_OPP", "USER_FRONT", "USER_BACK", "TARGET", "TARGET_OPP", "TARGET_FRONT", "TARGET_BACK"].include?(particle[:graphic])
      spr.src_rect.x = 0
    else
