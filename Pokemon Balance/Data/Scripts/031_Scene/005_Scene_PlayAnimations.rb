class Battle::Scene
  ...
  def pbFindMoveAnimation(moveID, idxUser, hitNum = 0)
    anims = find_move_animation(moveID, hitNum, idxUser)
    return nil if !anims
    if anims[0].is_a?(GameData::Animation)
      # New format, but we need old format, so find old animation
      anim = pbFindMoveAnimDetails(moveID, idxUser, hitNum)
      return anim
    else
      return anims
    end
