  ...
  def drawItem(index, _count, rect)
    ...
    if index == self.itemCount - 1
      ...
      if item_data.show_quantity? && !showing_register_icon
        ...
        xQty    = rect.x + rect.width - self.contents.text_size(qtytext).width - 50
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag, party = $player.party, choosing = false, filterproc = nil, resetpocket = true)
    ...
    @sprites["itemicon"] = ItemIconSprite.new(52, Graphics.height - 46, nil, @viewport)
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize(
      "", 80, 322, Graphics.width - 72 - 24, 128, @viewport
    )
  ...
  def pbRefresh
    ...
    @sprites["currentpocket"].x = 480 + ((pocketAcc) * 14) + pocketX[pocketAcc]
  ...
  def pbRefreshIndexChanged
    ...
    if showslider
      ...
      overlay.blt(610, y, @sliderbitmap.bitmap, Rect.new(36, 0, 36, 4))
      i = 0
      while i * 16 < boxheight - 4 - 18
        height = [boxheight - 4 - 18 - (i * 16), 16].min
        overlay.blt(610, y + 4 + (i * 16), @sliderbitmap.bitmap, Rect.new(36, 4, 36, height))
        i += 1
      end
      overlay.blt(610, y + boxheight - 18, @sliderbitmap.bitmap, Rect.new(36, 20, 36, 18))
    end
