class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    if self.status == newStatus && !ignoreStatus
      if showMessages
        ...
        when :FROSTBITE then msg = _INTL("¡{1} ya está helado!", pbThis)
        when :DROWSY    then msg = _INTL("¡{1} ya está somnoliento!", pbThis)
        end
    ...
    # Uproar immunity
    if newStatus == :DROWSY && !(hasActiveAbility?(:SOUNDPROOF) && !@battle.moldBreaker)
      @battle.allBattlers.each do |b|
        next if b.effects[PBEffects::Uproar] == 0
        @battle.pbDisplay(_INTL("¡El alboroto mantiene a {1} alerta!", pbThis(true))) if showMessages
        return false
      end
    end
    # Type immunities
    ...
    if immuneByAbility
      if showMessages
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          when :FROSTBITE then msg = _INTL("¡{1} no puede ser helado!", pbThis)
          when :DROWSY then msg = _INTL("¡{1} permanece despierto!", pbThis)
          end
        elsif immAlly
          case newStatus
          when :SLEEP
          ...
          when :PARALYSIS
            msg = _INTL("¡{1} no puede ser paralizado por la habilidad {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :FROZEN
            msg = _INTL("¡{1} no puede ser congelado por la habilidad {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :FROSTBITE
            msg = _INTL("¡{1} no puede ser helado por la habilidad {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)   
          when :DROWSY
            msg = _INTL("¡{1} permanece despierto por la habilidad {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)                           
          ...
          when :FROSTBITE then msg = _INTL("¡La habilidad {1} de {2} previene la helada!", pbThis, abilityName)
          when :DROWSY    then msg = _INTL("¡{1} permanece despierto por la habilidad {2}!", pbThis, abilityName)
          end
        end
        @battle.pbDisplay(msg)
  ...
  def pbInflictStatus(newStatus, newStatusCount = 0, msg = nil, user = nil)
    ...
    if msg && !msg.empty?
      ...
      when :DROWSY
        @battle.pbDisplay(_INTL("¡{1} está somnoliento!", pbThis))
      end
    end
  ...
  def pbContinueStatus
    ...
    when :FROZEN
      @battle.pbDisplay(_INTL("¡{1} está congelado!", pbThis))
    when :FROSTBITE
      @battle.pbDisplay(_INTL("¡{1} se resiente de la helada!", pbThis))
    when :DROWSY
      @battle.pbDisplay(_INTL("¡{1} está somnoliento! ¡No se puede mover!", pbThis))
    end
    PBDebug.log("[Status continues] #{pbThis}'s sleep count is #{@statusCount}") if self.status == :SLEEP
    PBDebug.log("[Status continues] #{pbThis}'s drowsy count is #{@statusCount}") if self.status == :DROWSY
  end
  ...
  def pbCureStatus(showMessages = true)
    ...
    if showMessages
      ...
      when :FROSTBITE then @battle.pbDisplay(_INTL("{1} se ha curado de la helada.", pbThis))
      when :DROWSY    then @battle.pbDisplay(_INTL("{1} se ha despertado.", pbThis))
      end
    end
